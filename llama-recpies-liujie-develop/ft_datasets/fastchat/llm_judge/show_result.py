...
def display_result_pairwise(args):
    ...
    print(f"Input file: {input_file}")
    df_all = pd.read_json(input_file, lines=True, dtype=False)
    ...
    for index, row in df_all.iterrows():
        ...
        if args.baseline_model is not None:
            if args.baseline_model not in [str(row["model_1"]), str(row["model_2"])]:
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bench-name", type=str, default="unified_misc24")
    parser.add_argument("--input-file", type=str)
    parser.add_argument("--judge-model", type=str, default="gpt-4")
    parser.add_argument("--baseline-model", type=str, default="2023_10_10_19")
    parser.add_argument(
        "--model-list",
        type=str,
        nargs="+",
